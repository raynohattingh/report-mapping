...
- **Fingerprint collision**: a proposed detection fingerprint that also matches an existing registered profile is caught by verify-on-approve and returned to review — two profiles must never both claim the same document shape (FR-024).
...
- **FR-024**: Draft profile proposals MUST include an auto-derived detection fingerprint (distinctive header text, page anatomy, table signature) as reviewable elements subject to the same confirm/correct/remove flow as extraction elements. Verify-on-approve (FR-022) MUST check that the exemplar(s) match the fingerprint AND that no existing registered profile also matches it; a collision returns the proposal to review. After approval, the Detect stage matches this shape deterministically with no per-batch human assignment.
- **FR-025**: Template validation rules MUST originate as reviewable proposal elements: the proposal pre-fills whatever the PDF itself declares (required flags, field kinds, option lists, maximum lengths), and the analyst adjusts and adds business rules (formats, allowed vocabularies referencing seed tables loaded as data) during the same review — no rule is registered without passing through human review, and fixed-layout templates (which declare nothing) receive their rules entirely from the analyst.

### Key Entities

- **Draft Profile Proposal**: The persisted output of analysing an unrecognised source PDF — proposed header fields, record-row locations, column/label assignments, per-record image regions, and a detection fingerprint, each with confidence and a per-element review state (proposed / confirmed / corrected / removed). Status: draft until approved or discarded.
...
### Session 2026-07-12 (superspec brainstorm, round 3)

Two registration-content gaps found by re-reading the spec against the pipeline stages; both resolved:

1. **Detection fingerprint** — draft proposals include an auto-derived detection fingerprint as reviewable elements; verify-on-approve checks the exemplar matches it and no existing profile collides, then Detect matches the shape deterministically with no per-batch human assignment (FR-024, fingerprint-collision edge case).
2. **Validation-rule provenance** — template rules start from what the PDF declares (required flags, kinds, option lists, max lengths), then the analyst adjusts and adds business rules referencing seed vocabularies during review; fixed-layout templates get rules entirely from the analyst (FR-025).

No further high-impact gaps identified — brainstorming considered complete across the three sessions; next step is planning.
