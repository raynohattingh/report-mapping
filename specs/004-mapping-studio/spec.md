...
- **FR-032a**: For profile proposals drafted from multiple exemplars, the primary exemplar is the workspace and a switcher flips the pane to any other exemplar with the current element overlaid there; every element shows its cross-exemplar agreement, and low agreement is conspicuously flagged.
...
- **FR-033a**: Spatial elements are visually editable on the PDF: dragging/resizing a proposed region records a correction (corrected payload with the new coordinates), and the analyst can draw NEW regions the detector missed — recorded as analyst-sourced elements in the existing element vocabulary (evidence source "analyst"). Analyst-drawn elements pass through the same review lifecycle and the same verify-on-approve proof as detected ones; nothing enters a registry unverified.
...
- **FR-035**: Studio approval of a proposal MUST run verify-on-approve; failures MUST show the persisted verify report per-check (exemplar, check, detail) with the proposal remaining draft; successes register the same registry row as CLI approval. Each failed check deep-links to the implicated element(s), spotlighted on the PDF; large mismatch lists (hundreds of per-field roundtrip failures are a real case) are grouped by page/pattern with the full list expandable.
- **FR-035a**: The approval dialog pre-fills the registry identity (profile key@version / template name@version) from the proposal's context — seeded re-onboarding suggests the seeded key with a bumped structural version — analyst-editable before approval; after a successful approval the studio offers the natural next step (e.g. start a mapping session using the newly registered artifact), never starting it automatically.
...
- **FR-037**: The analyst MUST be able to create onboarding drafts (profile from exemplars, template from a target PDF) from the studio, including the seeded drift-reonboarding path; all CLI rejections (unsupported PDF kinds with named workaround, fingerprint drift block, kind-misuse warnings) MUST surface with the same messages — a kind-misuse warning offers an explicit "proceed anyway" equivalent to the CLI's force flag.
- **FR-037a**: A drift-blocked document in a run view offers a one-action re-onboarding shortcut: it starts a seeded onboarding draft with the blocked document as exemplar, seeded from the profile it drifted away from (the existing seeded path, prefilled) — SafeCard's block verdict becomes the start of the fix, with no new business logic.
...
### Session 2026-07-14 (superspec brainstorm, round 3 — onboarding surfaces)

1. **Spatial editing depth**: full region editing on the PDF — drag/resize proposed regions as corrections AND draw new analyst-sourced elements for detector misses; everything still passes verify-on-approve (→ FR-033a).
2. **Multi-exemplar review**: primary exemplar as workspace + switcher to overlay the current element on any other exemplar; cross-exemplar agreement shown per element, low agreement flagged (→ FR-032a).
3. **Drift loop closed**: a drift-blocked document in a run view offers one-action seeded re-onboarding (prefilled seed-from path) — SafeCard block becomes the start of the fix (→ FR-037a).
4. **Defaults folded**: verify-failure checks deep-link to implicated elements with large mismatch lists grouped (→ FR-035); approval dialog pre-fills registry identity, seeded re-onboarding suggests a bumped structural version, successful approval offers — never auto-starts — the next step (→ FR-035a); kind-misuse warnings get an explicit "proceed anyway" (→ FR-037).
